...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(100.0, 1000.0),  # All numbers are approximate.
                rg.Point(22.9, 66.8),
                rg.Point(45, 111),
                rg.Point(75.0, 55),
                rg.Point(88, 33.3),
                rg.Point(102, 17.8)]
    circle = rg.Circle(rg.Point(100, 50), 15)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    list_of_centers = generate_points_on_circle(circle, number_of_points)
    for k in range(len(list_of_centers)):
        center = list_of_centers[k]
        radius = 10
        new_circle = rg.Circle(center, radius)
        new_circle.fill_color = color
        new_circle.attach_to(window)
        list_of_centers[k].attach_to(window)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  555 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 200)
    circle.outline_thickness = 3
    pizza(window, circle, 555, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    list_of_starts = generate_points_on_circle(circle, number_of_slices)
    for k in range(len(list_of_starts)):
        start = list_of_starts[k]
        end = circle.center
...
def run_test_polygon():
    ...
    # Test 3
    title = 'POLYGON test 3: 5 segments with thick magenta lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 175), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'magenta', 6)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = 'FANCY POLYGON test 4:  20 magenta lines on blue circle, ' \
            'hops = 5.'
    window = rg.RoseWindow(400, 350, title)

    circle = rg.Circle(rg.Point(240, 200), 175)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 20, 5, 'magenta', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(list_of_points)):
        start = list_of_points[k]
        end = list_of_points[(k + hops_to_next_point) % number_of_lines]
        line = rg.Line(start, end)
        line.color = color
        line.thickness = thickness
        line.arrow = 'last'
